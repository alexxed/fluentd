...
module Fluent::Plugin
  class SyslogInput < Input
    ...
    SYSLOG_REGEXP = /^\<([0-9]+)\>(.*)/
    SYSLOG_REGEXP_RFC3164 = /^<(?<pri>[0-9]+)>(?<time>[^ ]* {1,2}[^ ]* [^ ]*) (?<host>[a-zA-Z0-9\-.]*) (?<ident>[^ :\[]*)(?:\[(?<pid>[0-9]+)\])?(?:[^\:]*\:)? *(?<message>.*)$/
    SYSLOG_REGEXP_WITHOUT_HOST = /^<(?<pri>[0-9]+)>(?<time>[^ ]* {1,2}[^ ]* [^ ]*) (?<ident>[^ :\[]*)(?<rest>.*)$/
    ...
    config_param :protocol_type, :enum, list: [:tcp, :udp, :unix], default: :udp
    ...
    desc "The path to your Unix Domain Socket. The default is /dev/log."
    config_param :path, :string, default: '/dev/log'
    desc "The backlog of Unix Domain Socket. The default is 1024."
    ...
    def start
      ...
      log.info "listening syslog socket on #{@protocol_type == :unix ? @path : "#{@bind}:#{@port}"} with #{@protocol_type}"
      case @protocol_type
      when :udp then start_udp_server
      when :tcp then start_tcp_server
      when :unix then start_unix_socket
      else
    ...
    def start_unix_socket
      server_create_unix(:in_syslog_unix_server, nil, path: @path, backlog: @backlog, shared: false) do |data, sock|
        message_handler(add_host_if_missing(data.chomp("\x00")), sock)
      end
    end
    ...
    def add_host_if_missing(message)
      return message if Fluent::Plugin::SyslogParser::REGEXP_DETECT_RFC5424.match(message)
      return message if SYSLOG_REGEXP_RFC3164.match(message)

      m = SYSLOG_REGEXP_WITHOUT_HOST.match(message)
      return message unless m

      short_host = Socket.gethostname.split('.').first
      "<#{m[:pri]}>#{m[:time]} #{short_host} #{m[:ident]}#{m[:rest]}"
    end
  end
end
